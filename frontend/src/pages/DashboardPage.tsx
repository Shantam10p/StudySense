...
export default function DashboardPage() {
  ...
  /* ── Stat chips data for mobile ───────────────────────── */
  const statChips = [
    { icon: "dashboard", iconColor: "text-[#cdc0ec]", label: `${sessionCountValue} ${sessionCountLabel}` },
    { icon: "timer", iconColor: "text-[#8fa1a1]", label: `${sessionTimeValue}m Study` },
    { icon: "check_circle", iconColor: "text-[#edbbb1]", label: `${stats.completed} Completed` },
    { icon: "local_fire_department", iconColor: "text-[#dbcefb]", label: `${stats.streak} Day Streak`, fill: true },
  ];
  ...
  if (courses.length === 0) {
    return (
      <div className="flex min-h-screen bg-[#0e0e0e]">
        <Sidebar />
        <main className="md:ml-64 flex-1 flex flex-col pb-20 md:pb-0">

          {/* Mobile top bar */}
          <header className="flex md:hidden items-center gap-3 px-4 pt-4 pb-2">
            <button
              onClick={toggle}
              className="w-9 h-9 rounded-full bg-[#4b4166] flex items-center justify-center shrink-0"
            >
              <span className="material-symbols-outlined text-[#cdc0ec] text-xl">menu</span>
            </button>
            <span className="font-['Manrope'] font-bold text-xl text-[#cdc0ec]">Sensei</span>
          </header>

          {/* Desktop header */}
          <header className="hidden md:flex flex-col pt-8 pb-6 px-12 max-w-screen-2xl bg-transparent">
          ...
          <section className="md:hidden px-4 pt-4 pb-2">
            <h1 className="font-['Manrope'] font-extrabold text-3xl leading-tight text-[#e7e5e5]">
              {getGreeting()}, <span className="text-[#cdc0ec]">{userName}</span>
            </h1>
          </section>

          <div className="px-4 md:px-12 pb-8 max-w-screen-2xl space-y-6">
            {/* Stats — desktop grid / mobile chips */}
            ...
            <section className="bg-[#131313] rounded-xl border-2 border-[#2a2a2a] px-8 md:px-12 py-12 md:py-16 flex flex-col items-center text-center">
              <div className="w-16 md:w-20 h-16 md:h-20 rounded-2xl bg-[#cdc0ec]/10 flex items-center justify-center mb-6 md:mb-8">
                <span className="material-symbols-outlined text-[#cdc0ec] text-4xl md:text-5xl">auto_stories</span>
              </div>
  ...
  const renderSessionCard = (session: SessionWithCourse) => {
    ...
    return (
      <div
        ...
        className={`relative overflow-hidden shadow-xl shadow-black/20 border-2 rounded-xl transition-all duration-500 ${
          isLoading
      ...
      >
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {isLoading ? (
              ...
              <button
                ...
                className="flex items-center gap-1 text-[10px] font-medium text-[#767575] hover:text-[#e8956d] transition-colors disabled:opacity-40"
              >
        ...
        {/* Desktop layout */}
        <div className="hidden md:flex items-center justify-between py-6 px-6">
  ...
  return (
    <div className="flex min-h-screen bg-[#0e0e0e] overflow-x-hidden">
      ...
      <main className="md:ml-64 flex-1 flex flex-col overflow-x-hidden pb-24 md:pb-0 min-w-0">
        ...
        {/* ── Mobile greeting ────────────────────────────── */}
        <section className="md:hidden px-4 pt-2 pb-1">
          <h1 className="font-['Manrope'] font-extrabold text-3xl leading-tight text-[#e7e5e5]">
            {getGreeting()}, <span className="text-[#cdc0ec]">{userName}</span>
          </h1>
          <p className="text-[#acabaa] text-sm mt-1">
            {stats.sessionsToday > 0
              ? `${stats.sessionsToday} ${stats.sessionsToday === 1 ? "session" : "sessions"} scheduled today.`
        ...
        <div className="md:hidden px-4 pb-3">
          <div className="flex items-center bg-[#1f2020] rounded-full px-4 py-2.5 text-[#acabaa]">
            <span className="material-symbols-outlined mr-2 text-sm">search</span>
            <input
              className="bg-transparent border-none outline-none text-sm flex-1 focus:ring-0 placeholder:text-[#767575]"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
        ...
        <div className="px-4 md:px-12 pb-6 md:pb-8 max-w-screen-2xl md:grid md:grid-cols-12 md:gap-6">
          ...
          <div className="md:col-span-8 space-y-4 md:space-y-6">
            ...
            <section className="space-y-3 md:space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg md:text-xl font-['Manrope'] font-semibold text-[#e7e5e5]">Study Sessions</h3>
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setIsSessionMenuOpen((current) => !current)}
                    className="min-w-[120px] md:min-w-[152px] bg-[#131313] border-2 border-[#2a2a2a] text-[#e7e5e5] pl-3 md:pl-4 pr-8 md:pr-10 py-2 md:py-2.5 rounded-lg text-sm font-medium text-left transition-colors hover:border-[#cdc0ec]/40"
                  >
                    {sessionViewLabel}
                  </button>
                  <span className="material-symbols-outlined absolute right-2 md:right-3 top-1/2 -translate-y-1/2 text-[#acabaa] pointer-events-none text-lg">
                    {isSessionMenuOpen ? "expand_less" : "expand_more"}
                  </span>

                  {isSessionMenuOpen && (
                    <div className="absolute right-0 top-[calc(100%+8px)] z-20 min-w-[120px] md:min-w-[152px] overflow-hidden rounded-lg border-2 border-[#2a2a2a] bg-[#131313] shadow-xl shadow-black/30">
                      {[
              ...
              {filteredSessions.length === 0 ? (
                <div className="bg-[#131313] rounded-xl p-6 md:p-8 text-center border-2 border-[#2a2a2a]">
                  ...
                  <button
                    onClick={() => navigate("/planner/new")}
                    className="mt-4 px-6 py-2 bg-gradient-to-br from-[#cdc0ec] to-[#bfb2de] text-[#443b5f] font-semibold rounded-lg transition-all hover:scale-[1.02] text-sm"
                  >
                    Create Study Plan
                  </button>
                </div>
          ...
          <div className="md:col-span-4 md:space-y-6 mt-4 md:mt-0 mb-4 md:mb-0">
            ...
            {/* Upcoming schedule — desktop always visible / mobile collapsible */}
            <div className="bg-[#131313] rounded-xl border-2 border-[#2a2a2a]">
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="material-symbols-outlined text-[#cdc0ec] text-xl">calendar_today</span>
                  <span className="font-['Manrope'] font-bold text-[#e7e5e5]">Upcoming Schedule</span>
                </div>
                <span className={`material-symbols-outlined text-[#acabaa] transition-transform duration-300 ${isScheduleOpen ? "rotate-180" : ""}`}>
                  keyboard_arrow_down
                </span>
              </button>
              ...
              {/* Content — always on desktop, toggled on mobile */}
              <div className={`${isScheduleOpen ? "block" : "hidden"} md:block p-4 md:p-6 md:pt-0`}>
                ...
                <button
                  ...
                  className="w-full mt-6 md:mt-8 text-xs font-semibold text-[#cdc0ec] py-3 rounded-lg border border-[#cdc0ec]/20 hover:bg-[#cdc0ec]/5 transition-colors"
                >
                  View All Courses
                </button>
              </div>
